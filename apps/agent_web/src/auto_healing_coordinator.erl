...
start_link() ->
    colored_logger:ocean(deep, "[AUTO_HEAL] [TOOL] Starting Auto-Healing Coordinator"),
...
handle_call(disable_auto_healing, _From, State) ->
    colored_logger:warning("[AUTO_HEAL] Auto-healing disabled", []),
...
handle_cast({trigger_healing, Problem, Context}, State) ->
    NewState = case State#state.auto_healing_enabled of
        ...
        false ->
            colored_logger:warning("[AUTO_HEAL] Healing request ignored (auto-healing disabled): ~p", [Problem]),
            State
    end,
    {noreply, NewState};
...
terminate(_Reason, _State) ->
    colored_logger:warning("[AUTO_HEAL] Auto-Healing Coordinator terminating", []),
    ok.
...
determine_healing_strategy(Problem, Context) ->
    case Problem of
        ...
        websocket_disconnected ->
            reconnect_websocket;
        _ ->
            colored_logger:warning("[AUTO_HEAL] WARNING Unknown problem type: ~p, using generic strategy", [Problem]),
            generic_recovery
    end.

%% Execute healing strategy
execute_healing_strategy(HealingId, Strategy, Problem, Context) ->
    colored_logger:info("[AUTO_HEAL] [TOOL] Executing healing strategy ~p for ~s", [Strategy, HealingId]),
...
heal_supervisor_restart(Context) ->
    ...
    case SupervisorName of
        ...
        _ ->
            colored_logger:warning("[AUTO_HEAL] WARNING Don't know how to restart supervisor ~p", [SupervisorName]),
...
heal_generic_recovery(_Context) ->
    colored_logger:info("[AUTO_HEAL] [TOOL] Applying generic recovery measures"),
...
handle_healing_completion(HealingId, Result, State) ->
    case maps:get(HealingId, State#state.active_healings, undefined) of
        undefined ->
            colored_logger:warning("[AUTO_HEAL] WARNING Received completion for unknown healing ~s", [HealingId]),
            State;
        HealingInfo ->
